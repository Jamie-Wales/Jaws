...
The other optimisation that has been developed is constant folding. This process becomes easier at the ANF stage as all expressions are assigned to variables. We can guarante that  This allows for the easy identification of constant expressions that can be evaluated at compile time. This optimisation is crucial as it reduces the number of instructions that are ultimately generated.
...
\Procedure{DeadCodeElimination}{$Program$}
    ...
    \While{$WorkList \neq \emptyset$}
        ...
        \For{each edge $(node, dependent)$ in $G$}
            \If{$dependent \notin LiveNodes$}
                \State $\text{Enqueue}(WorkList, dependent)$
                \State $LiveNodes \gets LiveNodes \cup \{dependent\}$
            \EndIf
        \EndFor
    \EndWhile
    
    \State $G' \gets \text{FilterGraph}(G, LiveNodes)$ \Comment{Remove dead nodes and edges}
    \State $Program' \gets \text{RemoveDeadCode}(Program, LiveNodes)$

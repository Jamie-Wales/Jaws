      ...
      (syntax-rules ()
        ((do ((var init step ...) ...) (test expr ...) command ...)
         (letrec ((loop (lambda (var ...)
                          (if test
                              (begin (if #f #f) expr ...) ; R7RS needs placeholder for empty begin
    ...
    (define-syntax let-values
       (syntax-rules ()
         ((let-values (binding ...) body0 body1 ...)
          (let-values "bin" (binding ...) () (begin body0 body1 ...)))
         ((let-values "bin" () tmps body) (let tmps body))
         ((let-values "bin" ((b0 e0) binding ...) tmps body)
          (let-values "mktmp" b0 e0 () bindings tmps body))
         ((let-values "mktmp" () e0 args bindings tmps body)
          (call-with-values (lambda () e0) (lambda args (let-values "bin" bindings tmps body))))
         ((let-values "mktmp" (a . b) e0 (arg ...) bindings (tmp ...) body)
          (let-values "mktmp" b e0 (arg ... x) bindings (tmp ... (a x)) body))
         ((let-values "mktmp" a e0 (arg ...) bindings (tmp ...) body)
          (call-with-values (lambda () e0) (lambda (arg ... . x) (let-values "bin" bindings (tmp ... (a x)) body))))))

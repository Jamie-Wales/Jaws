...
(define-library
 ...
 (export filter
         ...
         remove
         partition
         ; List Predicates/Search/Manipulation HOFs
         compose
         identity
         constantly)
 ; Function Manipulation HOFs
 (import (base) (list))
 ...
 (begin
   ...
   (if (or (null? lst)
    ...
    (let ((pivot (car lst))
     ...
     (append (sort (filter (lambda (x)
                            (compare-proc x pivot))
                           rest)
                   compare-proc)
             (list pivot)
             (sort (filter (lambda (x)
                            (not (compare-proc x pivot)))
                           rest)
                   compare-proc)))))
  ...
  (define (any? pred lst)
   (let loop ((current-list lst))
    (cond
     ((null? current-list)
      #f)
     ; Reached end, none found
     ((pred (car current-list))
      #t)
  ...
  (define (every? pred lst)
   (let loop ((current-list lst))
    (cond
     ((null? current-list)
      #t)
     ; Reached end, all passed
     ((not (pred (car current-list)))
      #f)
     ; Found one failing, return false
     (else
      (loop (cdr current-list))))))
  ...
  (define (compose . procs)
   (if (null? procs)
    identity
    ; Composition of no functions is identity
    (let ((f (car procs))
          (rest-procs (cdr procs)))
     (if (null? rest-procs)
      f
      ; Composition of one function is the function itself
      ;; Recursively compose the rest, then compose f with that result
      (let ((g (apply compose rest-procs)))
       (lambda args
        ; Handle multiple arguments for the innermost function
        (f (apply g args))))))))

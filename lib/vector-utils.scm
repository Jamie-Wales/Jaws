(define-library (vector-utils)
  ...
  (begin
    ...
    (define (vector-map proc . vecs)
      ...
      (let loop ([vlist vecs] [min-len #f])
        (if (null? vlist)
            (let ([len min-len])
              (if (or (not len) (zero? len))
                  (vector)
                  (let ([result-vec (make-vector len)])
                    (let index-loop ([k 0]) ;; Start index loop
                      (if (< k len)
                          ;; THEN: Process index k and recurse
                          (begin
                            (let arg-loop ([vlist vecs] [current-args '()]) ;; Collect args for k
                              (if (null? vlist)
                            ...
                            (index-loop (+ k 1))) ;; Recurse for next index
            ...
            (let ([current-vec (car vlist)])
              (if (not (vector? current-vec))
                  ...
                  (let ([current-len (vector-length current-vec)])
                    (loop (cdr vlist) (if min-len (min min-len current-len) current-len))))))))
    ...
    (define (vector-append . vecs)
      (let loop ([vlist vecs] [total-len 0])
        (if (null? vlist)
            (let ([result-vec (make-vector total-len)])
              (let fill-loop ([vlist vecs] [current-idx 0])
                (if (null? vlist)
                    result-vec
                    (let ([current-vec (car vlist)])
                       (if (not (vector? current-vec))
                           ...
                           (let ([len (vector-length current-vec)])
                              (let copy-loop ([k 0] [idx current-idx])
                                 (if (< k len)
                                     (begin

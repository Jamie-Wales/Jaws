...
(define (filter pred lst)
  (cond ((null? lst) '())                           ; Base case: empty list
        ((pred (car lst))                           ; If predicate is true for first element
         (cons (car lst) (filter pred (cdr lst))))  ; Include it in result and recurse
        (else (filter pred (cdr lst)))))            ; Otherwise skip it and recurse

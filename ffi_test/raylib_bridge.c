...
{
    return (gameState.positions_x[0] < 0 || gameState.positions_x[0] >= gameState.grid_width || gameState.positions_y[0] < 0 || gameState.positions_y[0] >= gameState.grid_height);
}

bool bridge_check_self_collision()
{
    for (int i = 1; i < gameState.length; i++) {
        if (gameState.positions_x[0] == gameState.positions_x[i] && gameState.positions_y[0] == gameState.positions_y[i]) {
            return true;
        }
    }
    return false;
...
{
    return (gameState.positions_x[0] == gameState.food_x && gameState.positions_y[0] == gameState.food_y);
}
...
bool bridge_should_close()
{
    return WindowShouldClose();
}

void bridge_set_game_over()
{
    ...
    for (int i = 0; i < gameState.length; i++) {
        if (i == 0) {
            DrawRectangle(gameState.positions_x[i] * CELL_SIZE,
                gameState.positions_y[i] * CELL_SIZE,
...
void bridge_render_game_over()
{
    ...
    DrawText(TextFormat("SCORE: %d", gameState.score),
        width / 2 - MeasureText(TextFormat("SCORE: %d", gameState.score), 20) / 2,
        height / 2 + 20, 20, WHITE);

    EndDrawing();
}

void bridge_close_game()
{
    CloseWindow();
}
